...
Description: Wp-Feedburner is a plugin which deals with everything Feedburner.But for now,it will simply add your feedburner subscriber count to your Dashboard(in Right Now) irrespective of old or new feedburner URL.
Version: 0.1
Author: S.P.Arun
...
function wp_fb() {	
	//Add options page
	add_options_page('Wp Feedburner Settings', 'WP-FB Settings', 8, __FILE__, 'wp_fb_settings');
}
	...
	try {
		...
		if($count > 0){
			...
			jQuery(document).ready(function(){
				jQuery('#dashboard_right_now table tbody').before('<tr><td class="first b b-feed"><a href="http://feeds2.feedburner.com/<? echo $feeduri; ?>"><? echo $count; ?></a></td><td class="t feed"><a href="http://feeds2.feedburner.com/<? echo $feeduri; ?>">FeedBurner Subscribers</a></td></tr>');
			});
			</script>
			<?
		} else {
			...
			<script type="text/javascript">
			jQuery(document).ready(function(){
				jQuery('#dashboard_right_now table tbody').before('<tr><td class="first b b-feed"><a href="http://feeds2.feedburner.com/<? echo $feeduri; ?>">0</a></td><td class="t feed"><a href="http://feeds2.feedburner.com/<? echo $feeduri; ?>">FeedBurner Subscribers</a></td></tr>');
			});
			</script>
			<?
		}
...
function wp_fb_settings(){
	...
	<div class="wrap">
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="feeduri">Feedburner Id</label></th>
					<td>
						<input id="feeduri" type="text" name="feeduri" value="<?php echo $feeduri; ?>"  />
						<span class="setting-description">The bit that comes after the Feeburner URL.(If your Feed is http://feeds2.feedburner.com/s_p_arun or http://feeds.feedburner.com/s_p_arun , then "s_p_arun is your feedburner ID)" </span>
					</td>
				</tr>
				<tr>
					<th><label for="old">Check this If you use old feedburner URL</label></th>
					<td>
